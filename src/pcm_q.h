...
typedef enum _PcmQ_AccessFlags_t
{
  PcmQ_NoFlags            = 0x00,
  PcmQ_NoCountMutexForRead  = 0x01, // Use this flags if reading from an ISR (meaning no mutex access allowed)
  PcmQ_NoCountMutexForWrite = 0x02, // Use this flags if writing from an ISR (meaning no mutex access allowed)
  PcmQ_ReentrantWrites    = 0x04, // Use if multiple tasks may be writing
  PcmQ_ReentrantReads     = 0x08, // Use if multiple tasks may be reading
  PcmQ_NoMutexes          = 0x10  // Absolutely no mutexes should be used
} PcmQ_AccessFlags_t;
